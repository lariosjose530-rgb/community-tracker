...
export function useHabits(): UseHabitsResult {
  ...
  const loadHabits = useCallback(async () => {
    ...
    // CORRECCIÓN: Se agregó ': any' a log para satisfacer a TypeScript
    const completedIds = new Set((logRows ?? []).map((log: any) => log.habit_id));

    setHabits(
      // PREVENCIÓN: Se agregó ': any' a row por si acaso TypeScript también lo exige
      (habitRows ?? []).map((row: any) => ({
        id: row.id,
        title: row.title,
